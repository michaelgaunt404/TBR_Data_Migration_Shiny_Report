...
cleaned_table_view_columns = table_view_columns %>%  
  ...
  mutate(is_view = case_when(str_detect(TABLE_NAME, "v_")~"View", 
                                        T~"Table")) %>%   
  select(is_view, TABLE_SCHEMA, TABLE_NAME, COLUMN_NAME, rows) %>%  
  filter(rows>0)
...
checkboxGroupInput("schema", "Inlcuded Schemas:", inline = T,
                               choices = sort(unique(c(tmp$from_schema, tmp$to_schema))),
                               selected = c("lance", "xact", "rtoll", "qfree", "vehicle", "util"))
...
splitLayout(cellWidths = c("25%", "25%", "50%"),
            ...
            textInput("highlight", "Highlight a Node:", value = "place_holderz", placeholder = "highlight string")
...
# input = data.frame(spec = "nocp", 
#                    deg_sep = 2, 
#                    is_view = "table"
#                    )
# 
# srch_shcema =  c("lance", "xact", "rtoll", "qfree", "vehicle", "util")
# 
# 
# 
# 
# tmp %>%  
#   filter(str_detect(to_complete, input$spec) |
#            str_detect(from_complete, input$spec)) %>%  
#   filter(is_view %in% input$is_view)
# 
# 
# ##need a base to change for first thing you search for
# temp = tmp %>%  
#   filter(str_detect(to_complete, input$spec) |
#            str_detect(from_complete, input$spec)) %>%  
#   filter(is_view %in% input$is_view)
# 
# for (i in 1:input$deg_sep-1) {
#   
#   index = c(temp$to_complete,
#             temp$from_complete) %>%  
#     unique()
#   
#   temp = tmp %>%
#     dplyr::filter(to_complete %in% index | 
#                     from_complete %in% index) %>% 
#     filter(to_schema %in% srch_shcema, from_schema %in% srch_shcema,
#                   is_view %in% input$is_view
#     ) %>% 
#     bind_rows(temp, .) %>%  
#     unique()  
#     
# }
# 
# temp %>%  make_force_network(charge = -20)

renderForceNetwork({
  # tmp %>%  print()
tmp %>%
    dplyr::filter(to_schema %in% input$schema, from_schema %in% input$schema,
